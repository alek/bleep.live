...
class FunctionSpace8 extends Module {
	...
	getSpiralPoints(r, pointSize, rDelta, tDelta) {
		var result = []
		var inc = 0.1
		for (var theta = 0; theta < 360*32; theta+=tDelta) {
			var point = getViewport([xmax/2+r*Math.cos(deg2rad(theta)), ymax/2+r*Math.sin(deg2rad(theta))])
			// drawCircle(point, 2, "#fff", this.getDomID())
			// this.rect(ctx, xmax/2+r*Math.cos(deg2rad(theta)), ymax/2+r*Math.sin(deg2rad(theta)), 5, 5, "#fff")
			//this.circle(ctx, point[0], point[1], pointSize, "#fff")
	...
	render() {	
		...
		var points = this.getSpiralPoints(0, 3, 0.7, scale*5)
		...
		for (var i=0; i<points.length-30; i++) {
			...
			// this.line(ctx, points[i], points[i+10], "rgba(255,255,255," + Math.random() + ")")
			// this.line(ctx, points[i], points[i+15], "rgba(255,255,255," + Math.random() + ")")

			for (var j=0; j<20; j+=7) {
				this.line(ctx, points[i], points[i+j], "rgba(255,255,255," + 0.5 + ")")
			}

			// this.line(ctx, points[i], points[i+25], "rgba(255,255,255," + Math.random() + ")")
		...
		//drawCircle([xmax/2,ymax/2], ymax*0.2, "#000", this.getDomID())
		...
		// if (scale < 0.65) {
			// drawText([xmax/2, ymax/2], this.randomQuery()["term"], ymax*(1-scale*1.5) + "px", "#fff", 700, 0, "JetBrains Mono", this.getDomID())
		// }
