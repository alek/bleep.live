...
class StoryLine6 extends Module {
	...
	render() {	
		...
		var r = ymax*0.1
		for (var theta = this.objCount; theta<360*8; theta+=5) {
			var coord =  [xmax/2 + r*Math.cos(deg2rad(theta)), ymax/2 + r*Math.sin(deg2rad(theta)) ]
			drawCircle(coord, 2, this.primary, this.getDomID())	
			r+=5
		}
	...
	update(event) {
		...
		if (this.objCount > 45 && this.objCount < 50) {
			drawText([xmax/2,ymax/2], this.randomQuery()["term"], ymax*2 + "px", this.primary, 700, -5, "Helvetica", this.getDomID())		
		}
