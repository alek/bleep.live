...
class HDI2 extends Module {
	...
	gen() {
		...
		var coord = getGridCoordinates([3,4], this.params["grid_columns"], this.params["grid_rows"], xmax, ymax) 
		console.log(state["queue"][0])
		console.log(state["queue"][0]["payload"]["description"])

		var node = getTextLayoutNode(state["queue"][0]["payload"]["description"], 50, 50, 24, {
			x: coord[0],
			y: coord[1],
			"fill": "#fff",
