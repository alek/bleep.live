...
class TwitterFirehose4 extends Module {
	...
	render() {	
		...
		for (var i=0; i<36; i++) {
			var entry = data[Math.floor(timeSlide(10000,data.length-36))+i]
			if (entry != null) {
				text( { 
					x: xmax/2,
					y: ymax/2,
					"fill": "#fff",
					"transform": "rotate(" + i*10 + " " + xmax/2 + " " + ymax/2 + ")",
					"style": "font-size:" + (18 + timeRamp(7000,48)  )+ "px;text-align:left;alignment-baseline:left;text-anchor:left;opacity:1.0;font-family:Helvetica;sans-serif;font-weight:700;letter-spacing:-2px;"
				}, (entry["text"] == null) ? "" : entry["text"], this.getDomID()); 
			}
		}
		drawCircle([xmax/2,ymax/2], ymax*0.0001 + timeRamp(10000,ymax*0.1), "#000", this.getDomID())
...
export default TwitterFirehose4;
