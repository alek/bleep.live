...
class DNA1 extends Module {
	...
	dna(offset, amplitude, color) {
		...
		if (!color) { color = "#fff" }
		for (var i=offset; i<xmax+offset; i+=14) {
			var height = (Math.sin(deg2rad(i+this.counter))*amplitude)
			var lineW = 1
 			if (Math.random() < 0.02) {
 				lineW = 5
 				drawPath([ [i-offset, ymax/2 - amplitude - 10], 
 						   [i-offset, ymax/2 - amplitude -  60],
 						   [i-offset+20, ymax/2 - amplitude -  80] ], "#fff", Math.ceil(2*Math.random()), this.getDomID())
 				drawText([i-offset+40, ymax/2 - amplitude -  80], Math.random().toFixed(2), "10px", "#fff", 100, 0, "Roboto Mono", this.getDomID())
 			} else if (Math.random() < 0.05) {
 				lineW = 3
 				drawPath([ [i - offset,ymax/2+amplitude + 40], [i - offset,ymax/2+amplitude + 70]  ], "#fff", 20*Math.random(), this.getDomID() )
 			}
			drawLine([i - offset, ymax/2-height], [i - offset,ymax/2+height], "#fff", lineW, this.getDomID(), "", false, (lineW > 1) ? false : true)			
		}
		this.counter+=2
	}
