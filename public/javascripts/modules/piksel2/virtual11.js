...
class Virtual10 extends Module {
	...
	rndThread() {
		...
		path( {
			d: "M " + -200 + " " + ymax/2 + " t " + coords.map(x => x.join(" ")).join(" t ") + "",
	...
	render() {	
		...
		for (var i=0; i<xmax; i+=sqSize) {
			for (var j=0; j<ymax; j+=sqSize) {
				...
				if (Math.random() < 1.0) {
					for (var z=0; z<sqSize; z+=10) {
						drawRectangle([i+z,j], sqSize*0.05*Math.random(), sqSize*0.9, this.hexToRgbA(color, Math.random()), this.getDomID())
					}
					drawRectangleOutline([i,j], sqSize*0.9, sqSize*0.9, color, this.getDomID(), 2, 1, 2)
				}
			}
		}
