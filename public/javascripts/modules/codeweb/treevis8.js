...
class TreeVis8 extends Module {
	...
	render() {
		...
		// for (var i=0; i<360; i+= 20*Math.random()) {
		// 	new CircleSegment([xmax/2,ymax/2], i, i+10*Math.random(), 100 + 50*Math.floor(Math.random()*3), 50*Math.random(), randomPantoneHex()).draw(domID)	
		// }

		// new CircleSegment([xmax/2,ymax/2], 34, 40, 100, 30, randomPantoneHex()).draw(domID)
		...
		$.get("http://localhost:5133/api/tree" + filter, function( data ) {			
			...
			var angle = 0
			var angles = new Array(root.length).fill(0)
			...
			for (var i=0; i<root.length; i++) {
				...
				if (l1.is_file) {
					console.log(l1.value + "\t" + l1.is_file)
					new CircleSegment([xmax/2,ymax/2], angle, angle + 3, radius, 20, randomPantoneHex()).draw(domID)	
					angle += 4
				} else {

					var c = getCircleCoord(xmax/2,ymax/2, 0, radius + 15*i + 10)
					text({ 
							x: c[0]-10,
							y: c[1],
							"fill": "#fff",
							"transform": "rotate(" + 90 +  " " + c[0] + " " + c[1] + ")",
							"style": "font-size:10px;text-align:end;alignment-baseline:end;text-anchor:end;opacity:1.0;font-family:Roboto Mono;sans-serif;font-weight:100;letter-spacing:0px;"
					}, l1.value, domID); 

					console.log(l1.value)
					new CircleSegment([xmax/2,ymax/2], angle, angle + l1.children.length, radius, 20, randomPantoneHex()).draw(domID)	
					var subangle = 0
					for (var j=0; j<l1.children.length; j++) {
						var l2 = l1.children[j]
						var len = 1 + 5*Math.random()
						new CircleSegment([xmax/2,ymax/2], subangle, subangle + len, radius + 15*i, 12, randomPantoneHex()).draw(domID)	
						subangle += len + 1
						if (subangle > 270) {
							break
						}
			...
			var c = getCircleCoord(xmax/2,ymax/2, 0, 50 + 10)
