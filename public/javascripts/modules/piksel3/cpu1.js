...
class CPU1 extends Module {
	...
	render() {	
		...
		Piksel.addRectangleClip("glitchClip", xmax/2-maskWidth/2, ymax/2-maskWidth/2, maskWidth, maskWidth)
		...
		for (var i=0; i<Math.floor(this.params["r4"]/5); i++) {
			...
			line({
					...
					"stroke-width": xmax*this.getConfigVal("width", 0.05)*Math.random(),
	...
	// 		x: xmax/2,
	// 		y: ymax/2+20,
	// 		"fill": Math.random() < 0.1 ? "#000" : (Math.random() < 0.1 ? "gray" : "rgba(255,255,255," + Math.random() + ")"),
	// 		"clip-path": "url(#glitchClip)",
	// 		"transform": "rotate(" + 0 + " " + xmax/2 + " " + ymax/2 + ")",
	// 		"style": "font-size:" + r*3 + "px" + ";line-height:" + r + "px" + "text-align:middle;alignment-baseline:middle;text-anchor:middle;opacity:1.0;font-family:" + "Helvetica" + ";sans-serif;font-weight:" + sizes[this.params["w3"]%sizes.length] + ";letter-spacing:" + "-3" + "px;"
	// 	}, letters[Math.floor(Math.random()*letters.length)], this.getDomID()); 
	// }
