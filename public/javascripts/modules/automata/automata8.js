...
class Automata8 extends Module {
			...
			for (var j=0; j<this.height; j++) {
				...
				if (this.vector[offset] != null) {
					if (this.vector[offset]) {
						...
						// if (Math.random() < 0.2) {
							this.circle(ctx, i*this.sqSize, j*this.sqSize+this.sqSize/8, (j + i/2)%(this.sqSize/2), this.getColor())
						// } else {
						// 	this.rect(ctx, i*this.sqSize, j*this.sqSize, this.sqSize, this.sqSize/4, this.getColor())
						// }
					} else {
	...
	getVal(a1, a2, a3, a4, a5, a6) {
		var offset = parseInt('' + ~~a1 + ~~a2 + ~~a3 + ~~a4 + ~~a5 + ~~a6, 2)
