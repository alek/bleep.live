...
class Exoplanets4 extends Module {
	...
	render() {	
		...
		for (var i=0; i<this.max; i++) {
			for (var j=0; j<this.max; j++) {
				// this.set(i,j,0)
				this.set(i,j,Math.random() < 0.1 ? 64*Math.random() : 0)
				// this.set(i,j,j%Math.sqrt(j))
		...
		for (var i=0; i<this.max; i++) {
			for (var j=0; j<this.max; j++) {
				...
				if (coord[0] > 0 && coord[0] < xmax && coord[1] > 0 && coord[1] < ymax) {
					...
					if (j > 0) {
						if (this.get(i, j) == 0 && this.get(i, j-1) == 0) {
