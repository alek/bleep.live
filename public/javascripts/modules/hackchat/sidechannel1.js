...
class SideChannel1 extends Module {
		...
		$.get("http://localhost:5133/api/file" + filter, function( data ) {			
			...
			for (var i=0; i<content.length; i++) {
				...
				for (var j=1; j<content[i].length; j++) {
					var val = content[i][j]
					if (!colors[val]) {
						colors[val] = randomPantoneHex()
					}
					drawRectangle([xpos, ypos], dim-1, dim-1, colors[val], domID)
					xpos += dim
				}
				// xpos = (xpos + dim)%
				...
				if (i > 0 && ( ((i+1) % 7) == 0)) {
					// drawCircle([xpos, ypos], 10, "red", domID)
					// drawText([xpos + 10, ypos+5], i, "8px", "#fff", 100, 0, "Helvetica", domID)
					drawText([xoffset-10, ypos+5], content[i][0], "6px", "#fff", 100, 0, "Roboto Mono", domID, "end")
					drawText([xpos+10, ypos+5], content[i][0], "6px", "#fff", 100, 0, "Roboto Mono", domID, "start")
					xpos = xoffset
					ypos = ypos + dim
				}
			}
