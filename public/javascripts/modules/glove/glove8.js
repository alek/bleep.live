	...
	renderGrid(columns, rows) {
		for (var i=0; i<=columns; i++) {
			for (var j=0; j<=rows; j++) {
				...
				drawCircle(coord, timeSlide(1000,10), "#fff", this.getDomID())
				if (i > 0 && j>0) {
					if (i%Math.floor(timeSlide(1000,7)) == 0) {
						drawLine(getViewport(getGridCoordinates([i,j], columns, rows, xmax, ymax)), 
							getViewport(getGridCoordinates([i-1,j], columns, rows, xmax, ymax)),
							"#fff", "1px", this.getDomID() )
					}
					if (j%Math.floor(timeSlide(1000,11)) == 0) {
						drawLine(getViewport(getGridCoordinates([i,j], columns, rows, xmax, ymax)), 
							getViewport(getGridCoordinates([i,j-1], columns, rows, xmax, ymax)),
							"#fff", "1px", this.getDomID() )
					}
				}
			}
